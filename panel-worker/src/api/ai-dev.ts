...
import { aiDevChat, aiDevListDir, aiDevReadFile, aiDevWriteFile, AiDevMessage } from "../lib/ai-dev";
...
export async function aiDevChatApi(env: Env, token: string, history: unknown, attachedFiles: unknown, message: string) {
	await gateAdmin(env, token);
	...
	const files = Array.isArray(attachedFiles)
		...
		: [];
	const out = await aiDevChat(env, cfg, hist, files, String(message ?? ""));
